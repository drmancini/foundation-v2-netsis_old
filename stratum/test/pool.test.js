    ...
    pool.on('pool.log', (type, text) => {
      if (type !== 'debug') {
        ...
        if (response.length === 2) {
          ...
          expect(response[0][1]).toBe('Submitted a primary block (Netsis:1) successfully to Metsis\'s daemon instance(s)');
  ...
  test('Test pool manager setup [9]', (done) => {
    ...
    pool.on('pool.log', (type, text) => {
      if (type !== 'debug') {
        ...
        expect(text).toBe('Submitted a primary block (Netsis:1) successfully to Netsis\'s daemon instance(s)');
        done();
      }
    });
    mockSetupDaemons(pool, () => {
